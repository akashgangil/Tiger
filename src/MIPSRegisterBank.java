...
public class MIPSRegisterBank{
    ...
    static {
        ...
        initRegBank.put("ARGS", new RegManager (Arrays.asList("$a0", "$a1", "$a2", "$a3"), true));
        initRegBank.put("TEMPS", new RegManager (Arrays.asList("$t0", "$t1", "$t2", "$t3", "$t4", "$t5", "$t6", "$t7", "$t8", "$t9"), true));
        initRegBank.put("SAVED_TEMP", new RegManager (Arrays.asList("$s0", "$s1", "$s2", "$s3", "$s4", "$s5", "$s6", "$s7"), true));
